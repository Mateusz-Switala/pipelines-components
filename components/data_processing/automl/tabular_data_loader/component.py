...
def automl_data_loader(  # noqa: D417
    ...
    **100** valid records must remain; otherwise the component fails with a clear error
    ...
    # A larger batch reduces Python/pandas dispatch and DataFrame-copy overhead while
    # remaining small relative to the loader's 8 GiB requested memory.
    ...
    with status:
        ...
        def load_data_in_batches(
            ...
            # Representative samplers must inspect the full population. Downloading
            # it once with boto3's multipart transfer manager avoids feeding a
            # multi-gigabyte object through one HTTP stream and lets pandas parse
            # from node-local storage. Keep the streaming fallback for endpoints
            # that do not implement multipart downloads.
            if sampling_method in {"random", "stratified"}:
                ...
                try:
                    ...
                    if bytes_downloaded == 0:
                        raise ValueError("multipart download produced an empty file")
                    io_metrics.update(
                        {
                            "s3_transfer_mode": "multipart_local",
                            "source_bytes": bytes_downloaded,
                            "download_seconds": round(time.monotonic() - transfer_started, 3),
                        }
                    )
                    if sampling_method == "stratified":
                        return (
                            _sample_stratified(local_path, PANDAS_CHUNK_SIZE, max_size_bytes, label_column),
                            io_metrics,
                        )
                    ...
                    # pandas has completed its synchronous read before this point.
                    if local_path:
                        try:
